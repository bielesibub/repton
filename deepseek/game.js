...
function drawNameEntry() {
  clearScreen(); setDefaultColours();
  drawLogoSmall();
  printString('Congratulations. ' + String.fromCharCode(0x83) + 'Enter your name', 2, 12);
  printString(nameBuffer + '_', 4, 14);
  printString('Last Score : ' + String(score).padStart(6, '0'), 4, 16);
}
function submitName() {
...
window.addEventListener('keydown', e => {
  ...
  else if (state === ST.MAP && e.code === 'Space') { enterStatusScreen(); e.preventDefault(); }
...
function statusKey(e) {
  ...
  else if (e.code === 'KeyM' && screenNum < 8) { state = ST.MAP; drawMiniMap(); }
